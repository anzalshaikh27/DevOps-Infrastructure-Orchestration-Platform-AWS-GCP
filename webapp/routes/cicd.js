...
const checkPayload = (req, res, next) => {
  ...
  if (errorMessage) {
    ...
    logError(error, 'CICD Check - Bad Request');
    
    // Track the API call with error status
    metricsCollector.trackAPICall('CICDCheck', Date.now() - startTime, 'error');
...
router.all('/', setHeaders, (req, res, next) => {
  if (req.method !== 'GET') {
    ...
    const error = new Error(`Method ${req.method} not allowed on cicd endpoint`);
    logError(error, 'CICD Check - Method Not Allowed');
    
    // Track the API call with error status
    metricsCollector.trackAPICall('CICDCheck', 0, 'error');
...
router.get('/', setHeaders, checkPayload, async (req, res) => {
  ...
  try {
    ...
    await metricsCollector.trackDatabaseQuery('CICDCheckCreation', dbDuration);

    res.status(200).end();

    // Log request and track API metrics
    logRequest(req, res, 'CICD Check', startTime);
    await metricsCollector.trackAPICall('CICD', Date.now() - startTime);
  } catch (error) {
    // Comprehensive error logging
    logError(error, 'CICD Check');
    await metricsCollector.trackAPICall('CICDCheck', Date.now() - startTime, 'error');
    res.status(503).end();
  }
});
